...
from models.model_ViT import vit_classifier
#model = vit_classifier(x_train, num_classes)

from models.model_2DCNN import cnn_2D
#model = cnn_2D(x_train, num_classes)

from models.model_morpmamba import MorphMamba
#model =  MorphMamba(x_train, 32 , Num_Classes = num_classes)

from models.model_MLP import MLP
#model = MLP(x_train, num_classes = num_classes)

from models.model_3DCNN import cnn_3D
#model = cnn_3D(img_list = x_train, num_class = num_classes)
...
for i in range(10):
    ...
    checkpoint = ModelCheckpoint(
        f"Model_Weights/{DATASET}/model_{i}.h5",
    ...
    #model.load_weights(f"Model_Weights/{DATASET}/model_{i}.h5")    
    Y_pred = predict_by_batching(model, input_tensor_idx = test_idx, batch_size = 1000, X = data, windowSize = window_size)
...
#i = np.argmin(np.abs(Oa -  np.mean(Oa)))
#model.load_weights(f"Model_Weights/{DATASET}/model_{i}.h5")

Predicted_Class_Map = get_class_map(model, data, ref_data, window_size)
